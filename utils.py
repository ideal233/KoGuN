...
def exp_name(args, exclude_list, seed, seperator='-', exp_flag=''):
    d = vars(args)
    name = ''
    start = True
    for arg in d:
        ...
        if start:
            ...
            name += seperator + arg + '=' + str(d[arg])
        start = False
    if exp_flag != '':
        name = exp_flag + seperator + name
    cur_time_str = datetime.datetime.now().strftime('%m%d%H%M')
    name = name + '/seed=' + str(seed) + seperator + cur_time_str
    # name = os.path.join(name, 'seed=' + str(seed) + seperator + cur_time_str)
    # print(name)
    return name
